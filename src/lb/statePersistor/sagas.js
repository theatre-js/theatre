...
const pathToPersistenceFile =
  path.join(app.getPath('userData'), 'state.json')

const whitelistOfPartsOfStateToPersist = [
  ['projects'],
]
...
function* loadState(): Generator<*, *, *> {
  ...
  const content = yield call(fse.readFile, pathToPersistenceFile, {encoding: 'utf-8'})
...
function* persistNewState(newState: {}): Generator<*, *, *> {
  ...
  yield call(fse.writeFile, pathToPersistenceFile, stringified, {encoding: 'utf-8'})
}