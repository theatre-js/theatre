...
describe('createNewProject()', () => {
  ...
  it('should work for an existing folder', async () => {
    const {task, store} = await runSingleSaga(createNewProject, {folderPath: '/foo/baz', name: 'baz'})
    const result = await task.done
    expect(result).toMatchObject({type: 'ok', filePath: '/foo/baz/theaterjs.json'})
  ...
  it('should error for a non-existing folder', async () => {
    const {task} = await runSingleSaga(createNewProject, {folderPath: '/foo/quz', name: 'baz'})
    const result = await task.done
    expect(result).toMatchObject({type: 'error', errorType: 'folderDoesntExist'})
  })

  it('should error for a folder that already has a theaterjs.json file', async () => {
    const {task} = await runSingleSaga(createNewProject, {folderPath: '/foo/bar', name: 'baz'})
    const result = await task.done
    expect(result).toMatchObject({type: 'error', errorType: 'theaterjsDotJsonFileAlreadyExists'})
  })

  it('should error for a non-folder', async () => {
    const {task} = await runSingleSaga(createNewProject, {folderPath: '/foo/bar/theaterjs.json', name: 'baz'})
    const result = await task.done
