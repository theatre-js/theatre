...
export function* removePointFromLane(laneId: LaneID, atIndex: number): Generator<*, void, *> {
  yield reduceState(['animationTimeline', 'lanes', 'byId', laneId, 'points'], (points) => (
...
export function* changePointPositionBy(laneId: LaneID, atIndex: number, change: PointPosition): Generator<*, void, *> {
  yield reduceState(['animationTimeline', 'lanes', 'byId', laneId, 'points', atIndex], (point) => ({
    ...point,
    t: point.t + change.t,
...
export function* changePointHandlesBy(laneId: LaneID, atIndex: number, change: PointHandles): Generator<*, void, *> {
  yield reduceState(['animationTimeline', 'lanes', 'byId', laneId, 'points', atIndex, 'handles'], (handles) => {
    return handles.map((handle, index) => (handle + change[index]))
  })
  yield * resetExtremums(laneId)
}

export function* makeHandleHorizontal(laneId: LaneID, atIndex: number, side: 'right' | 'left'): Generator<*, void, *> {
  yield reduceState(['animationTimeline', 'lanes', 'byId', laneId, 'points', atIndex, 'handles'], (handles) => {
    if (side === 'left') {
      handles[0] = Math.sign(handles[0]) * Math.sqrt(Math.pow(handles[0], 2) + Math.pow(handles[1], 2))
      handles[1] = 0
    }
    if (side === 'right') {
      handles[2] = Math.sign(handles[2]) * Math.sqrt(Math.pow(handles[2], 2) + Math.pow(handles[3], 2))
      handles[3] = 0
    }
    return handles
  })
...
export function* makeHandlesParallel(laneId: LaneID, atIndex: number, side: 'right' | 'left'): Generator<*, void, *> {
  yield reduceState(['animationTimeline', 'lanes', 'byId', laneId, 'points', atIndex, 'handles'], (handles) => {
    if (side === 'left') {
...
export function* makeHandlesEqual(laneId: LaneID, atIndex: number, side: 'right' | 'left'): Generator<*, void, *> {
  yield reduceState(['animationTimeline', 'lanes', 'byId', laneId, 'points', atIndex, 'handles'], (handles) => {
    if (side === 'left') {
      handles[0] = - handles[2]
      handles[1] = - handles[3]
