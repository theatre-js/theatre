...
import BoxView from '$theater/AnimationTimelinePanel/BoxesContainer/BoxView'
...
class BoxesContainer extends PureComponentWithTheater<IProps, IState> {
  ...
  grab: TBoxDnDAPI['grab'] = (index: number, height: number, top: number) => {
    this.setState(() => ({
      status: 'grab',
      grabProps: {index, height, top},
    }))
  }

  move: TBoxDnDAPI['move'] = (index: number, top: number) => {
    this.setState(() => ({
      status: 'move',
      dropProps: {index, top},
    }))
  ...
  merge: TBoxDnDAPI['merge'] = (index: number, height: number, top: number) => {
    this.setState(() => ({
      status: 'merge',
