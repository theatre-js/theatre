...
import * as utils from '$theater/AnimationTimelinePanel/SelectionProvider/utils'
import {
  TDims,
...
class SelectionProvider extends PureComponentWithTheater<IProps, IState> {
              ...
              Object.keys(variableInfo).forEach((pointKey: string) => {
                ...
                variables = set(
                  path,
                  {
                    ...
                    value: pointProps.value - (move.y / boxHeight) * extDiff,
                  },
                  variables,
                )
              })
  ...
  _getBoxesBoundaries(): TBoxesBoundaries {
    const boxes: BoxesObject = val(get(
      this.theater.atom2.pointer,
      this.props.pathToTimeline.concat('boxes'),
    ) as Pointer<BoxesObject>)
    const layout: LayoutArray = val(get(
      this.theater.atom2.pointer,
      this.props.pathToTimeline.concat('layout'),
    ) as Pointer<LayoutArray>)
    return utils.memoizedGetBoxesBoundaries(boxes, layout)
  }
  ...
  activateSelection = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    this.setState(() => ({
      ...
      boxesBoundaries: this._getBoxesBoundaries(),
    }))
  }
